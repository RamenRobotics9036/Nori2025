...
public class IntakeSystem extends SubsystemBase{
    ...
    private double maxOutput = IntakeConstants.maxOutputPercentage;

    //sets the idle mode of both motors to kBrake
    public IntakeSystem(){
        config.idleMode(SparkBaseConfig.IdleMode.kBrake);
        m_pullMotor.configure(config, SparkBase.ResetMode.kResetSafeParameters, SparkBase.PersistMode.kPersistParameters);
        m_loadMotor.configure(config, SparkBase.ResetMode.kResetSafeParameters, SparkBase.PersistMode.kPersistParameters);
    }
